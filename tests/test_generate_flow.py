...
# ── the public/private arm hook ──────────────────────────────

def test_public_arms_filters_arms_marked_private(monkeypatch):
    """The hook works — tested against a synthetic registry, so it does not depend on
    production data hiding anything."""
    monkeypatch.setattr(generation, "load_prefixes", lambda: {
        "base": {"sequence": "", "label": "No prefix"},
        "shown": {"sequence": "x", "label": "Shown"},
        "hidden": {"sequence": "y", "label": "Hidden", "public": False},
    })
    assert {a["arm"] for a in generation.public_arms()} == {"base", "shown"}


def test_every_shipped_arm_is_public():
    """Both poles are served. An anti arm withheld while the pro arms ship would make
